...
async function limitGlobalDaily(req, res, next) {
    try {
      ...
      `);
        ...
        if (rows.length === 0) {
            await db.query('INSERT INTO global_limit (id, count, last_request) VALUES (1, $1, $2)', [1, now]);
            return next();
        }
        ...
        if (timePassed > ONE_DAY) {
            await db.query('UPDATE global_limit SET count = 1, last_request = $1 WHERE id = 1', [now]);
            return next();
        }
        if (count >= 2) {
            return res.status(429).send('Limite diário de uso da API atingido. Tente novamente amanhã.');
        }
        await db.query('UPDATE global_limit SET count = count + 1, last_request = $1 WHERE id = 1', [now]);
        next();
    }
    catch (err) {
...
exports.default = limitGlobalDaily;
